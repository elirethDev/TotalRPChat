...
3. Maintain the target entry in `CHANGELOG.md` as `Unreleased` during development and prepare matching release notes.
4. During development, run `pwsh -File tools/release-check.ps1 -ExpectedVersion <version>`.
5. Run the Lua bytecode checks, `git diff --check`, and the gameplay/runtime checks below.
6. Before final publication, convert the version heading to a dated release and run `pwsh -File tools/release-check.ps1 -ExpectedVersion <version> -RequireReleased`.
7. Review the package contents and compatibility metadata.
8. Create the Git tag and GitHub release, then publish the same reviewed package to Steam Workshop.
...
- Keep Project Zomboid compatibility separate from the release version. `42/mod.info` provides the B42 `versionMin` constraint.
...
- [ ] Keep the development entry as `## [<version>] - Unreleased` while changes remain.
- [ ] Before tagging or publishing, convert it to `## [<version>] - <YYYY-MM-DD>` using the final release date.
...
- [ ] Do not tag or publish while the target version still has an `Unreleased` heading; final publication requires the dated heading and `-RequireReleased` check.

## B42 compatibility

- [ ] Confirm `42/mod.info` is present and retains the B42 `versionMin` value.
- [ ] Test the package in the supported B42 environment.
- [ ] Verify that the B42 package layout keeps the mod descriptor and implementation under `42/`.

## Static and runtime validation

- [ ] Run `pwsh -File tools/release-check.ps1 -ExpectedVersion <version>`.
- [ ] For final publication only, run `pwsh -File tools/release-check.ps1 -ExpectedVersion <version> -RequireReleased`.
