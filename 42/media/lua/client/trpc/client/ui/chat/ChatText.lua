...
function ChatText:render()
    ...
    for c, v in ipairs(self.images) do
        self:drawTextureScaled(
            v,
            self.imageX[c] + self.marginLeft,
            self.imageY[c] + self.marginTop,
            self.imageW[c],
            self.imageH[c],
            self.contentTransparency,
            1,
            1,
            1
        )
    ...
    while c <= #self.lines do
        ...
        if
            self.marginTop
                + self:getYScroll()
                + self.lineY[c]
                + getTextManager():getFontHeight(self.font)
                + self:getHeight() * 2
            > 0
        then
            ...
            if string.trim(v) ~= "" then
                if orient == "centre" then
                    ...
                    while (c <= #self.lines) and (self.lineY[c] == lineY) do
                        ...
                        self:drawText(
                            string.trim(self.lines[c]),
                            lineX + self.lineX[c],
                            self.lineY[c] + self.marginTop,
                            r,
                            g,
                            b,
                            self.contentTransparency,
